...
struct slice {
    ...
    reference at(size_type idx) const { 
        if (idx >= len) throw std::out_of_range("slice::at");
        return data[idx]; 
    }
    reference front() const noexcept { return data[0]; }
    reference back() const noexcept { return data[len - 1]; }
    
    // Iterators
    iterator begin() const noexcept { return data; }
    iterator end() const noexcept { return data + len; }
    const_iterator cbegin() const noexcept { return data; }
    const_iterator cend() const noexcept { return data + len; }
    ...
    slice(T (&arr)[N]) noexcept : data(arr), len(N) {}

#ifdef VELLUM_USE_SPAN
    operator std::span<T>() const noexcept { return std::span<T>(data, len); }
    slice(std::span<T> s) noexcept : data(s.data()), len(s.size()) {}
#endif
};
...
struct owned_slice {
    ...
    explicit owned_slice(size_type count, const T& value = T())
        : slice_data(new T[count], count), deleter([](T* p) noexcept { delete[] p; }) {
    ...
    const_reverse_iterator crbegin() const noexcept { return slice_data.crbegin(); }
    ...
    size_type max_size() const noexcept { return slice_data.max_size(); }
    ...
    owned_slice(const std::vector<T>& vec)
        : slice_data(nullptr, vec.size()), deleter([](T* p) noexcept { delete[] p; }) {
        if (!vec.empty()) {
...
template<typename T>
typename slice<T>::iterator begin(const slice<T>& s) noexcept { return s.begin(); }
template<typename T>
typename slice<T>::iterator end(const slice<T>& s) noexcept { return s.end(); }
template<typename T>
typename slice<T>::pointer data(const slice<T>& s) noexcept { return s.data; }
template<typename T>
typename slice<T>::size_type size(const slice<T>& s) noexcept { return s.size(); }
template<typename T>
bool empty(const slice<T>& s) noexcept { return s.empty(); }

template<typename T>
typename owned_slice<T>::iterator begin(const owned_slice<T>& s) noexcept { return s.begin(); }
template<typename T>
typename owned_slice<T>::iterator end(const owned_slice<T>& s) noexcept { return s.end(); }
template<typename T>
typename owned_slice<T>::pointer data(const owned_slice<T>& s) noexcept { return s.data(); }
template<typename T>
typename owned_slice<T>::size_type size(const owned_slice<T>& s) noexcept { return s.size(); }
template<typename T>
bool empty(const owned_slice<T>& s) noexcept { return s.empty(); }

template<typename T>
void swap(owned_ptr<T>& a, owned_ptr<T>& b) noexcept { a.swap(b); }
template<typename T>
void swap(owned_slice<T>& a, owned_slice<T>& b) noexcept { a.swap(b); }
...
namespace detail {
    ...
    template<typename T>
    using owned_selector = std::conditional_t<is_slice<T>::value, owned_slice<typename element<T>::type>, owned_ptr<std::remove_pointer_t<T>>>;
}

template<typename T>
using owned = detail::owned_selector<T>;
